...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/ip/access-list',
]

def deserialize_Access_list__icmp_cfg__icmp_src_cfg_json(obj):
	if obj is None:
		return None
	...
	result = Access_list__icmp_cfg__icmp_src_cfg(any_src_icmp=any_src_icmp, host_src_icmp=host_src_icmp, subnet_src_icmp=subnet_src_icmp, mask_src_icmp=mask_src_icmp)
	return result

def deserialize_Access_list__icmp_cfg__code_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Access_list__icmp_cfg__code(any_code=any_code, icmp_code=icmp_code)
	return result

def deserialize_Access_list__icmp_cfg__type__type_code_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Access_list__icmp_cfg__type__type_code(type_any_code=type_any_code, type_icmp_code=type_icmp_code, type_any_src=type_any_src, type_host_src=type_host_src, type_subnet_src=type_subnet_src, type_mask_src=type_mask_src)
	return result

def deserialize_Access_list__icmp_cfg__type__dest_unreach_cfg__dest_un_code_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
...
def deserialize_Access_list__icmp_cfg__type__dest_unreach_cfg_json(obj):
	...
	dest_un_code = deserialize_Access_list__icmp_cfg__type__dest_unreach_cfg__dest_un_code_json(data.get('dest-un-code'))
	...
	result = Access_list__icmp_cfg__type__dest_unreach_cfg(dest_unreachable=dest_unreachable, dest_un_code=dest_un_code, dest_un_any_src=dest_un_any_src, dest_un_host_src=dest_un_host_src, dest_un_subnet_src=dest_un_subnet_src, dest_un_mask_src=dest_un_mask_src)
	return result

def deserialize_Access_list__icmp_cfg__type_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
...
def deserialize_Access_list__icmp_cfg_json(obj):
	...
	if isinstance(obj, dict):
		...
		data = json.loads(obj)
	icmp = data.get('icmp')
	icmp_src_cfg = deserialize_Access_list__icmp_cfg__icmp_src_cfg_json(data.get('icmp-src-cfg'))
	code = deserialize_Access_list__icmp_cfg__code_json(data.get('code'))
	type = deserialize_Access_list__icmp_cfg__type_json(data.get('type'))
	result = Access_list__icmp_cfg(icmp=icmp, icmp_src_cfg=icmp_src_cfg, code=code, type=type)
	return result

def deserialize_Access_list__ip_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Access_list__ip_cfg(ip=ip, any_src_ip=any_src_ip, host_src_ip=host_src_ip, subnet_src_ip=subnet_src_ip, mask_src_ip=mask_src_ip)
	return result

def deserialize_Access_list__any_code_any_source_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Access_list__any_code_any_source_cfg(any_code_any_src=any_code_any_src, any_code_host_src=any_code_host_src, any_code_subnet_src=any_code_subnet_src, any_code_mask_src=any_code_mask_src)
	return result

def deserialize_Access_list__dst_ip_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	dscp = data.get('dscp')
	log = data.get('log')
	result = Access_list__dst_ip_cfg(any_dst_ip=any_dst_ip, host_dst_ip=host_dst_ip, subnet_dst_ip=subnet_dst_ip, mask_dst_ip=mask_dst_ip, fragments=fragments, vlan=vlan, dscp=dscp, log=log)
	return result

def deserialize_Access_list__l4_cfg__l4_src_cfg__l4_src_port_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Access_list__l4_cfg__l4_src_cfg__l4_src_port_cfg(eq_src=eq_src, gt_src=gt_src, lt_src=lt_src, range_src=range_src, port_num_end_src=port_num_end_src)
	return result

def deserialize_Access_list__l4_cfg__l4_src_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	l4_src_port_cfg = deserialize_Access_list__l4_cfg__l4_src_cfg__l4_src_port_cfg_json(data.get('l4-src-port-cfg'))
	result = Access_list__l4_cfg__l4_src_cfg(any_src_tcp=any_src_tcp, host_src_tcp=host_src_tcp, subnet_src_tcp=subnet_src_tcp, mask_src_tcp=mask_src_tcp, l4_src_port_cfg=l4_src_port_cfg)
	return result

def deserialize_Access_list__l4_cfg__l4_dst_cfg__l4_dst_port_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Access_list__l4_cfg__l4_dst_cfg__l4_dst_port_cfg(eq_dst=eq_dst, gt_dst=gt_dst, lt_dst=lt_dst, range_dst=range_dst, port_num_end_dst=port_num_end_dst)
	return result

def deserialize_Access_list__l4_cfg__l4_dst_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	l4_dst_port_cfg = deserialize_Access_list__l4_cfg__l4_dst_cfg__l4_dst_port_cfg_json(data.get('l4-dst-port-cfg'))
	result = Access_list__l4_cfg__l4_dst_cfg(any_dst_tcp=any_dst_tcp, host_dst_tcp=host_dst_tcp, subnet_dst_tcp=subnet_dst_tcp, mask_dst_tcp=mask_dst_tcp, l4_dst_port_cfg=l4_dst_port_cfg)
	return result

def deserialize_Access_list__l4_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	tcp = data.get('tcp')
	udp = data.get('udp')
	l4_src_cfg = deserialize_Access_list__l4_cfg__l4_src_cfg_json(data.get('l4-src-cfg'))
	l4_dst_cfg = deserialize_Access_list__l4_cfg__l4_dst_cfg_json(data.get('l4-dst-cfg'))
	tcp_fragments = data.get('tcp-fragments')
	vlan = data.get('vlan')
	dscp = data.get('dscp')
	established = data.get('established')
	log = data.get('log')
	result = Access_list__l4_cfg(tcp=tcp, udp=udp, l4_src_cfg=l4_src_cfg, l4_dst_cfg=l4_dst_cfg, tcp_fragments=tcp_fragments, vlan=vlan, dscp=dscp, established=established, log=log)
	return result
...
def deserialize_Access_list_json(obj):
	...
	icmp_cfg = deserialize_Access_list__icmp_cfg_json(data.get('icmp-cfg'))
	ip_cfg = deserialize_Access_list__ip_cfg_json(data.get('ip-cfg'))
	any_code_any_source_cfg = deserialize_Access_list__any_code_any_source_cfg_json(data.get('any-code-any-source-cfg'))
	dst_ip_cfg = deserialize_Access_list__dst_ip_cfg_json(data.get('dst-ip-cfg'))
	l4_cfg = deserialize_Access_list__l4_cfg_json(data.get('l4-cfg'))
	result = Access_list(name=name, seq_num=seq_num, action=action, remark=remark, icmp_cfg=icmp_cfg, ip_cfg=ip_cfg, any_code_any_source_cfg=any_code_any_source_cfg, dst_ip_cfg=dst_ip_cfg, l4_cfg=l4_cfg)
	return result

def serialize_Access_list__icmp_cfg__icmp_src_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__icmp_cfg__code_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__icmp_cfg__type__type_code_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__icmp_cfg__type__dest_unreach_cfg__dest_un_code_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__icmp_cfg__type__dest_unreach_cfg_json(obj):
	output = OrderedDict()
	if obj.dest_unreachable is not None:
		output['dest-unreachable'] = obj.dest_unreachable
	if obj.dest_un_code is not None:
		output['dest-un-code'] = serialize_Access_list__icmp_cfg__type__dest_unreach_cfg__dest_un_code_json(obj.dest_un_code)
...
def serialize_Access_list__icmp_cfg__type_json(obj):
	output = OrderedDict()
	...
	if obj.type_code is not None:
		output['type-code'] = serialize_Access_list__icmp_cfg__type__type_code_json(obj.type_code)
	if obj.dest_unreach_cfg is not None:
		output['dest-unreach-cfg'] = serialize_Access_list__icmp_cfg__type__dest_unreach_cfg_json(obj.dest_unreach_cfg)
	return output

def serialize_Access_list__icmp_cfg_json(obj):
	output = OrderedDict()
	if obj.icmp is not None:
		output['icmp'] = obj.icmp
	if obj.icmp_src_cfg is not None:
		output['icmp-src-cfg'] = serialize_Access_list__icmp_cfg__icmp_src_cfg_json(obj.icmp_src_cfg)
	if obj.code is not None:
		output['code'] = serialize_Access_list__icmp_cfg__code_json(obj.code)
	if obj.type is not None:
		output['type'] = serialize_Access_list__icmp_cfg__type_json(obj.type)
	return output

def serialize_Access_list__ip_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__any_code_any_source_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__dst_ip_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__l4_cfg__l4_src_cfg__l4_src_port_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__l4_cfg__l4_src_cfg_json(obj):
	output = OrderedDict()
	...
	if obj.l4_src_port_cfg is not None:
		output['l4-src-port-cfg'] = serialize_Access_list__l4_cfg__l4_src_cfg__l4_src_port_cfg_json(obj.l4_src_port_cfg)
	return output

def serialize_Access_list__l4_cfg__l4_dst_cfg__l4_dst_port_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Access_list__l4_cfg__l4_dst_cfg_json(obj):
	output = OrderedDict()
	...
	if obj.l4_dst_port_cfg is not None:
		output['l4-dst-port-cfg'] = serialize_Access_list__l4_cfg__l4_dst_cfg__l4_dst_port_cfg_json(obj.l4_dst_port_cfg)
	return output

def serialize_Access_list__l4_cfg_json(obj):
	output = OrderedDict()
	...
	if obj.l4_src_cfg is not None:
		output['l4-src-cfg'] = serialize_Access_list__l4_cfg__l4_src_cfg_json(obj.l4_src_cfg)
	if obj.l4_dst_cfg is not None:
		output['l4-dst-cfg'] = serialize_Access_list__l4_cfg__l4_dst_cfg_json(obj.l4_dst_cfg)
...
def serialize_Access_list_json(obj):
	...
	if obj.icmp_cfg is not None:
		output['icmp-cfg'] = serialize_Access_list__icmp_cfg_json(obj.icmp_cfg)
	if obj.ip_cfg is not None:
		output['ip-cfg'] = serialize_Access_list__ip_cfg_json(obj.ip_cfg)
	if obj.any_code_any_source_cfg is not None:
		output['any-code-any-source-cfg'] = serialize_Access_list__any_code_any_source_cfg_json(obj.any_code_any_source_cfg)
	if obj.dst_ip_cfg is not None:
		output['dst-ip-cfg'] = serialize_Access_list__dst_ip_cfg_json(obj.dst_ip_cfg)
	if obj.l4_cfg is not None:
		output['l4-cfg'] = serialize_Access_list__l4_cfg_json(obj.l4_cfg)
	return output
...
class Access_list__icmp_cfg__icmp_src_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__icmp_cfg__code(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__icmp_cfg__type__type_code(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__icmp_cfg__type__dest_unreach_cfg__dest_un_code(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__icmp_cfg__type__dest_unreach_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__icmp_cfg__type(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__icmp_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__ip_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__any_code_any_source_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__dst_ip_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__l4_cfg__l4_src_cfg__l4_src_port_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__l4_cfg__l4_src_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__l4_cfg__l4_dst_cfg__l4_dst_port_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__l4_cfg__l4_dst_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Access_list__l4_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class IpAccesslistClient(AbstractResourceClient):
	...
	def getIpAccesslist(self, access_list_name):
		"""
	...
	def putIpAccesslist(self, access_list_name, access_list):
		"""
	...
	def deleteIpAccesslist(self, access_list_name):
		"""
...
class AllIpAccesslistsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitIpAccesslist(self, access_list):
		"""
