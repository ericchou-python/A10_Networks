...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/ipv6/prefix-list',
]

def deserialize_Ipv6_prefix_list_rules__action_cfg__ipaddr_cfg_json(obj):
	if obj is None:
		return None
	...
	ge = data.get('ge')
	le = data.get('le')
	result = Ipv6_prefix_list_rules__action_cfg__ipaddr_cfg(ipaddr=ipaddr, ge=ge, le=le)
	return result

def deserialize_Ipv6_prefix_list_rules__action_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	any = data.get('any')
	ipaddr_cfg = deserialize_Ipv6_prefix_list_rules__action_cfg__ipaddr_cfg_json(data.get('ipaddr-cfg'))
	result = Ipv6_prefix_list_rules__action_cfg(action=action, any=any, ipaddr_cfg=ipaddr_cfg)
	return result

def deserialize_Ipv6_prefix_list_rules_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
		...
		data = json.loads(obj)
	seq = data.get('seq')
	action_cfg = deserialize_Ipv6_prefix_list_rules__action_cfg_json(data.get('action-cfg'))
	result = Ipv6_prefix_list_rules(seq=seq, action_cfg=action_cfg)
	return result

def deserialize_list_Ipv6_prefix_list_rules_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Ipv6_prefix_list_rules_json(item))
...
def deserialize_Prefix_list__name_cfg_json(obj):
	...
	if isinstance(obj, dict):
		...
		data = json.loads(obj)
	name = data.get('name')
	rules = deserialize_list_Ipv6_prefix_list_rules_json(data.get('rules'))
...
def serialize_Ipv6_prefix_list_rules__action_cfg__ipaddr_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Ipv6_prefix_list_rules__action_cfg_json(obj):
	output = OrderedDict()
	...
	if obj.ipaddr_cfg is not None:
		output['ipaddr-cfg'] = serialize_Ipv6_prefix_list_rules__action_cfg__ipaddr_cfg_json(obj.ipaddr_cfg)
	return output

def serialize_Ipv6_prefix_list_rules_json(obj):
	output = OrderedDict()
	if obj.seq is not None:
		output['seq'] = obj.seq
	if obj.action_cfg is not None:
		output['action-cfg'] = serialize_Ipv6_prefix_list_rules__action_cfg_json(obj.action_cfg)
	return output

def serialize_final_json(obj):
	return json.dumps(obj)

def serialize_list_Ipv6_prefix_list_rules_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Ipv6_prefix_list_rules_json(item))
	return output
...
def serialize_Prefix_list__name_cfg_json(obj):
	...
	if obj.rules is not None:
		output['rules'] = serialize_list_Ipv6_prefix_list_rules_json(obj.rules)
	return output
...
class Ipv6_prefix_list_rules__action_cfg__ipaddr_cfg(AxapiObject):
	__metaclass__ = StructMeta 
	ipaddr=SizedString(1, 255)
	ge=PosRangedInteger(0, 128)
	le=PosRangedInteger(0, 128)
...
class Ipv6_prefix_list_rules__action_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ipv6_prefix_list_rules(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ipv6PrefixlistClient(AbstractResourceClient):
	...
	def getIpv6Prefixlist(self, prefix_list_name_cfg):
		"""
	...
	def putIpv6Prefixlist(self, prefix_list_name_cfg, prefix_list):
		"""
	...
	def deleteIpv6Prefixlist(self, prefix_list_name_cfg):
		"""
...
class AllIpv6PrefixlistsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitIpv6Prefixlist(self, prefix_list):
		"""
	...
	def getAllIpv6Prefixlists(self):
		"""
