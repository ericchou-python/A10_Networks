########################################################################################################################
# File name: interface_trunk_bfd.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/interface/trunk/bfd',
]
...
def deserialize_Interface_trunk_bfd_multiplier_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Interface_trunk_bfd_multiplier(multiplier_value=multiplier_value)
	return result

def deserialize_list_Interface_trunk_bfd_multiplier_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_trunk_bfd_multiplier_json(item))
...
def deserialize_Bfd__interval_cfg__min_rx_json(obj):
	...
	multiplier = deserialize_list_Interface_trunk_bfd_multiplier_json(data.get('multiplier'))
...
def serialize_Interface_trunk_bfd_multiplier_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_trunk_bfd_multiplier_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_trunk_bfd_multiplier_json(item))
	return output
...
def serialize_Bfd__interval_cfg__min_rx_json(obj):
	...
	if obj.multiplier is not None:
		output['multiplier'] = serialize_list_Interface_trunk_bfd_multiplier_json(obj.multiplier)
	return output
...
class Interface_trunk_bfd_multiplier(AxapiObject):
	__metaclass__ = StructMeta 
...
class InterfaceTrunkBfdClient(AbstractResourceClient):
	...
	def putInterfaceTrunkBfd(self, bfd):
		"""
	...
	def deleteInterfaceTrunkBfd(self):
		"""
...
class AllInterfaceTrunkBfdsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitInterfaceTrunkBfd(self, bfd):
		"""
	...
	def getAllInterfaceTrunkBfds(self):
		"""
