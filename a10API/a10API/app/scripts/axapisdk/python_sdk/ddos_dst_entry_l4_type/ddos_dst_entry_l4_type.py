########################################################################################################################
# File name: ddos_dst_entry_l4_type.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/ddos/dst/entry/l4-type',
]
...
def deserialize_L4_type__exceed_action_json(obj):
	...
	result = L4_type__exceed_action(exceed_drop=exceed_drop, exceed_black_list=exceed_black_list)
	return result

def deserialize_Ddos_dst_entry_l4_type_key_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
		...
		data = json.loads(obj)
	key = data.get('key')
	result = Ddos_dst_entry_l4_type_key_cfg(key=key)
	return result

def deserialize_list_Ddos_dst_entry_l4_type_key_cfg_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Ddos_dst_entry_l4_type_key_cfg_json(item))
...
def deserialize_L4_type__tunnel_decap_json(obj):
	...
	key_cfg = deserialize_list_Ddos_dst_entry_l4_type_key_cfg_json(data.get('key-cfg'))
...
def serialize_Ddos_dst_entry_l4_type_key_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Ddos_dst_entry_l4_type_key_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Ddos_dst_entry_l4_type_key_cfg_json(item))
	return output
...
def serialize_L4_type__tunnel_decap_json(obj):
	...
	if obj.key_cfg is not None:
		output['key-cfg'] = serialize_list_Ddos_dst_entry_l4_type_key_cfg_json(obj.key_cfg)
	return output
...
class Ddos_dst_entry_l4_type_key_cfg(AxapiObject):
	__metaclass__ = StructMeta 
	key=SizedString(1, 8)
...
class DdosDstEntryL4typeClient(AbstractResourceClient):
	...
	def getDdosDstEntryL4type(self, l4_type_protocol):
		"""
	...
	def putDdosDstEntryL4type(self, l4_type_protocol, l4_type):
		"""
	...
	def deleteDdosDstEntryL4type(self, l4_type_protocol):
		"""
...
class AllDdosDstEntryL4typesClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitDdosDstEntryL4type(self, l4_type):
		"""
	...
	def getAllDdosDstEntryL4types(self):
		"""
