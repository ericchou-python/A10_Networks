########################################################################################################################
# File name: snmp_server_enable_traps.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/snmp-server/enable/traps',
]
...
def deserialize_Traps_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Traps(all=all, lldp=lldp, snmp=snmp, ha=ha, system=system, network=network)
	return result
...
def deserialize_list_Traps_json(obj):
	if obj is None:
		return None
	if isinstance(obj, list):
		data = obj
	...
	container = []
	for item in data:
		container.append(deserialize_Traps_json(item))
	return list(container)

class Traps(AxapiObject):
...
class SnmpserverEnableTrapsClient(AbstractResourceClient):
	...
	def getSnmpserverEnableTraps(self):
		"""
		Retrieve the traps identified by the specified identifier
		
		Returns:
			An instance of the Traps class
		"""
		query = ''
		...
		expected_status = 200
		errors = {500: 'An unexpected runtime exception', 404: 'Specified traps does not exist'}
		...
		if payload is not None:
			...
			payload= data.get('traps')
		return deserialize_Traps_json(payload)

	def putSnmpserverEnableTraps(self, traps):
		"""
		Replace the object traps
		
		Args:
			traps An instance of the Traps class
		...
		headers = { 'Content-type' : 'application/json', 'Authorization' : 'A10 %s' %self.sessionid}
		output=OrderedDict()
		output['traps']=serialize_Traps_json(traps)
	...
	def deleteSnmpserverEnableTraps(self):
		"""
		Remove the traps identified by the specified identifier from the system
...
class AllSnmpserverEnableTrapssClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitSnmpserverEnableTraps(self, traps):
		"""
		Create the object traps
	...
	def getAllSnmpserverEnableTrapss(self):
		"""
		Retrieve all traps objects currently pending in the system
		...
		if payload is not None:
			data = json.loads(payload)
			payload= data.get('trapsList')
		return deserialize_list_Traps_json(payload)
