########################################################################################################################
# File name: interface_trunk_ip.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/interface/trunk/ip',
]

def deserialize_Interface_trunk_ip_address_val_cfg_json(obj):
	if obj is None:
		return None
	...
	result = Interface_trunk_ip_address_val_cfg(address_val=address_val, netmask=netmask)
	return result

def deserialize_list_Interface_trunk_ip_address_val_cfg_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_trunk_ip_address_val_cfg_json(item))
...
def deserialize_Ip__address_json(obj):
	...
	if isinstance(obj, dict):
		...
		data = json.loads(obj)
	address_val_cfg = deserialize_list_Interface_trunk_ip_address_val_cfg_json(data.get('address_val-cfg'))
	dhcp = data.get('dhcp')
...
def deserialize_Ip_json(obj):
	...
	result = Ip(address=address, cache_spoofing_port=cache_spoofing_port, helper_address=helper_address, nat=nat, generate_membership_query=generate_membership_query, generate_membership_query_val=generate_membership_query_val, max_resp_time=max_resp_time)
	return result

def serialize_Interface_trunk_ip_address_val_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_trunk_ip_address_val_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_trunk_ip_address_val_cfg_json(item))
	return output

def serialize_Ip__address_json(obj):
	output = OrderedDict()
	if obj.address_val_cfg is not None:
		output['address_val-cfg'] = serialize_list_Interface_trunk_ip_address_val_cfg_json(obj.address_val_cfg)
...
class Interface_trunk_ip_address_val_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class InterfaceTrunkIpClient(AbstractResourceClient):
	...
	def putInterfaceTrunkIp(self, ip):
		"""
	...
	def deleteInterfaceTrunkIp(self):
		"""
...
class AllInterfaceTrunkIpsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitInterfaceTrunkIp(self, ip):
		"""
	...
	def getAllInterfaceTrunkIps(self):
		"""
