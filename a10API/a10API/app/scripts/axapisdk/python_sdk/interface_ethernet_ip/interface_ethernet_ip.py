########################################################################################################################
# File name: interface_ethernet_ip.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/interface/ethernet/ip',
]

def deserialize_Interface_ethernet_ip_ip_cfg_json(obj):
	if obj is None:
		return None
	...
	result = Interface_ethernet_ip_ip_cfg(ipv4_address=ipv4_address, ipv4_netmask=ipv4_netmask)
	return result

def deserialize_list_Interface_ethernet_ip_ip_cfg_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_ethernet_ip_ip_cfg_json(item))
...
def deserialize_Ip__address_json(obj):
	...
	if isinstance(obj, dict):
		...
		data = json.loads(obj)
	ip_cfg = deserialize_list_Interface_ethernet_ip_ip_cfg_json(data.get('ip-cfg'))
	dhcp = data.get('dhcp')
	result = Ip__address(ip_cfg=ip_cfg, dhcp=dhcp)
	return result
...
def serialize_Interface_ethernet_ip_ip_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_ethernet_ip_ip_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_ethernet_ip_ip_cfg_json(item))
	return output

def serialize_Ip__address_json(obj):
	output = OrderedDict()
	if obj.ip_cfg is not None:
		output['ip-cfg'] = serialize_list_Interface_ethernet_ip_ip_cfg_json(obj.ip_cfg)
...
class Interface_ethernet_ip_ip_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class InterfaceEthernetIpClient(AbstractResourceClient):
	...
	def getInterfaceEthernetIp(self):
		"""
	...
	def putInterfaceEthernetIp(self, ip):
		"""
	...
	def deleteInterfaceEthernetIp(self):
		"""
...
class AllInterfaceEthernetIpsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitInterfaceEthernetIp(self, ip):
		"""
	...
	def getAllInterfaceEthernetIps(self):
		"""
