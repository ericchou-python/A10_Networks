...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/interface/ve',
]

def deserialize_Ve__snmp_cfg_json(obj):
	if obj is None:
		return None
	...
	result = Ve__snmp_cfg(snmp_server=snmp_server, trap_source=trap_source)
	return result

def deserialize_Ve__icmp_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Ve__icmp_cfg(icmp_rate_limit=icmp_rate_limit, normal_val=normal_val, lockup=lockup, lockup_period=lockup_period)
	return result

def deserialize_Ve__icmpv6_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	lockup = data.get('lockup')
	lockup_period = data.get('lockup-period')
	result = Ve__icmpv6_cfg(icmpv6_rate_limit=icmpv6_rate_limit, normal_val=normal_val, lockup=lockup, lockup_period=lockup_period)
	return result

def deserialize_Interface_ve_address_val_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Interface_ve_address_val_cfg(address_val=address_val, netmask=netmask)
	return result

def deserialize_list_Interface_ve_address_val_cfg_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_ve_address_val_cfg_json(item))
	return list(container)

def deserialize_Ve__ip__address_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
		...
		data = json.loads(obj)
	address_val_cfg = deserialize_list_Interface_ve_address_val_cfg_json(data.get('address_val-cfg'))
	dhcp = data.get('dhcp')
	result = Ve__ip__address(address_val_cfg=address_val_cfg, dhcp=dhcp)
	return result

def deserialize_Ve__ip__nat_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Ve__ip__nat(inside=inside, outside=outside)
	return result

def deserialize_Ve__ip_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
		...
		data = json.loads(obj)
	address = deserialize_Ve__ip__address_json(data.get('address'))
	helper_address = data.get('helper-address')
	nat = deserialize_Ve__ip__nat_json(data.get('nat'))
	...
	result = Ve__ip(address=address, helper_address=helper_address, nat=nat, generate_membership_query=generate_membership_query, generate_membership_query_val=generate_membership_query_val, max_resp_time=max_resp_time)
	return result

def deserialize_Interface_ve_address_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Interface_ve_address_cfg(ipv6_addr=ipv6_addr, anycast=anycast, link_local=link_local)
	return result

def deserialize_list_Interface_ve_address_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, list):
		data = obj
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_ve_address_cfg_json(item))
	return list(container)

def deserialize_Ve__ipv6__nat_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	inside = data.get('inside')
	outside = data.get('outside')
	result = Ve__ipv6__nat(inside=inside, outside=outside)
	return result

def deserialize_Ve__ipv6__ndisc__router_adver__mtu_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Ve__ipv6__ndisc__router_adver__mtu(adver_mtu=adver_mtu, adver_mtu_disable=adver_mtu_disable)
	return result

def deserialize_Interface_ve_prefix_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Interface_ve_prefix_cfg(prefix=prefix, not_autonomous=not_autonomous, not_on_link=not_on_link, preferred_lifetime=preferred_lifetime, valid_lifetime=valid_lifetime)
	return result

def deserialize_list_Interface_ve_prefix_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, list):
		data = obj
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_ve_prefix_cfg_json(item))
	return list(container)

def deserialize_Ve__ipv6__ndisc__router_adver__vrid_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Ve__ipv6__ndisc__router_adver__vrid(adver_vrid=adver_vrid, adver_vrid_default=adver_vrid_default, use_floating_ip=use_floating_ip, floating_ip=floating_ip)
	return result

def deserialize_Ve__ipv6__ndisc__router_adver__ha_group_id_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Ve__ipv6__ndisc__router_adver__ha_group_id(adver_ha_group_id=adver_ha_group_id, ha_use_floating_ip=ha_use_floating_ip, ha_floating_ip=ha_floating_ip)
	return result

def deserialize_Ve__ipv6__ndisc__router_adver_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	mtu = deserialize_Ve__ipv6__ndisc__router_adver__mtu_json(data.get('mtu'))
	prefix_cfg = deserialize_list_Interface_ve_prefix_cfg_json(data.get('prefix-cfg'))
	vrid = deserialize_Ve__ipv6__ndisc__router_adver__vrid_json(data.get('vrid'))
	ha_group_id = deserialize_Ve__ipv6__ndisc__router_adver__ha_group_id_json(data.get('ha-group-id'))
	result = Ve__ipv6__ndisc__router_adver(adver_enable=adver_enable, adver_disable=adver_disable, default_lifetime=default_lifetime, hop_limit=hop_limit, max_interval=max_interval, min_interval=min_interval, rate_limit=rate_limit, reachable_time=reachable_time, retransmit_timer=retransmit_timer, mtu=mtu, prefix_cfg=prefix_cfg, vrid=vrid, ha_group_id=ha_group_id)
	return result

def deserialize_Ve__ipv6__ndisc_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
		...
		data = json.loads(obj)
	router_adver = deserialize_Ve__ipv6__ndisc__router_adver_json(data.get('router-adver'))
	result = Ve__ipv6__ndisc(router_adver=router_adver)
	return result

def deserialize_Ve__ipv6_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
		...
		data = json.loads(obj)
	address_cfg = deserialize_list_Interface_ve_address_cfg_json(data.get('address-cfg'))
	ipv6_enable = data.get('ipv6-enable')
	nat = deserialize_Ve__ipv6__nat_json(data.get('nat'))
	ndisc = deserialize_Ve__ipv6__ndisc_json(data.get('ndisc'))
	result = Ve__ipv6(address_cfg=address_cfg, ipv6_enable=ipv6_enable, nat=nat, ndisc=ndisc)
	return result

def deserialize_Ve__ddos_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	outside = data.get('outside')
	inside = data.get('inside')
	result = Ve__ddos(outside=outside, inside=inside)
	return result
...
def deserialize_Ve_json(obj):
	...
	mtu = data.get('mtu')
	snmp_cfg = deserialize_Ve__snmp_cfg_json(data.get('snmp-cfg'))
	action = data.get('action')
	icmp_cfg = deserialize_Ve__icmp_cfg_json(data.get('icmp-cfg'))
	icmpv6_cfg = deserialize_Ve__icmpv6_cfg_json(data.get('icmpv6-cfg'))
	ip = deserialize_Ve__ip_json(data.get('ip'))
	ipv6 = deserialize_Ve__ipv6_json(data.get('ipv6'))
	ddos = deserialize_Ve__ddos_json(data.get('ddos'))
	result = Ve(ifnum=ifnum, name=name, l3_vlan_fwd_disable=l3_vlan_fwd_disable, mtu=mtu, snmp_cfg=snmp_cfg, action=action, icmp_cfg=icmp_cfg, icmpv6_cfg=icmpv6_cfg, ip=ip, ipv6=ipv6, ddos=ddos)
	return result

def serialize_Ve__snmp_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Ve__icmp_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Ve__icmpv6_cfg_json(obj):
	output = OrderedDict()
...
def serialize_Interface_ve_address_val_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_ve_address_val_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_ve_address_val_cfg_json(item))
	return output

def serialize_Ve__ip__address_json(obj):
	output = OrderedDict()
	if obj.address_val_cfg is not None:
		output['address_val-cfg'] = serialize_list_Interface_ve_address_val_cfg_json(obj.address_val_cfg)
	if obj.dhcp is not None:
		output['dhcp'] = obj.dhcp
	return output

def serialize_Ve__ip__nat_json(obj):
	output = OrderedDict()
...
def serialize_Ve__ip_json(obj):
	output = OrderedDict()
	if obj.address is not None:
		output['address'] = serialize_Ve__ip__address_json(obj.address)
	if obj.helper_address is not None:
		output['helper-address'] = obj.helper_address
	if obj.nat is not None:
		output['nat'] = serialize_Ve__ip__nat_json(obj.nat)
...
def serialize_Interface_ve_address_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_ve_address_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_ve_address_cfg_json(item))
	return output

def serialize_Ve__ipv6__nat_json(obj):
	output = OrderedDict()
	if obj.inside is not None:
		output['inside'] = obj.inside
	if obj.outside is not None:
...
def serialize_Ve__ipv6__ndisc__router_adver__mtu_json(obj):
	output = OrderedDict()
...
def serialize_Interface_ve_prefix_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_ve_prefix_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_ve_prefix_cfg_json(item))
	return output

def serialize_Ve__ipv6__ndisc__router_adver__vrid_json(obj):
	output = OrderedDict()
...
def serialize_Ve__ipv6__ndisc__router_adver__ha_group_id_json(obj):
	output = OrderedDict()
...
def serialize_Ve__ipv6__ndisc__router_adver_json(obj):
	output = OrderedDict()
	...
	if obj.mtu is not None:
		output['mtu'] = serialize_Ve__ipv6__ndisc__router_adver__mtu_json(obj.mtu)
	if obj.prefix_cfg is not None:
		output['prefix-cfg'] = serialize_list_Interface_ve_prefix_cfg_json(obj.prefix_cfg)
	if obj.vrid is not None:
		output['vrid'] = serialize_Ve__ipv6__ndisc__router_adver__vrid_json(obj.vrid)
	if obj.ha_group_id is not None:
		output['ha-group-id'] = serialize_Ve__ipv6__ndisc__router_adver__ha_group_id_json(obj.ha_group_id)
	return output

def serialize_Ve__ipv6__ndisc_json(obj):
	output = OrderedDict()
	if obj.router_adver is not None:
		output['router-adver'] = serialize_Ve__ipv6__ndisc__router_adver_json(obj.router_adver)
	return output

def serialize_Ve__ipv6_json(obj):
	output = OrderedDict()
	if obj.address_cfg is not None:
		output['address-cfg'] = serialize_list_Interface_ve_address_cfg_json(obj.address_cfg)
	if obj.ipv6_enable is not None:
		output['ipv6-enable'] = obj.ipv6_enable
	if obj.nat is not None:
		output['nat'] = serialize_Ve__ipv6__nat_json(obj.nat)
	if obj.ndisc is not None:
		output['ndisc'] = serialize_Ve__ipv6__ndisc_json(obj.ndisc)
	return output

def serialize_Ve__ddos_json(obj):
	output = OrderedDict()
	if obj.outside is not None:
		output['outside'] = obj.outside
	if obj.inside is not None:
...
def serialize_Ve_json(obj):
	...
	if obj.snmp_cfg is not None:
		output['snmp-cfg'] = serialize_Ve__snmp_cfg_json(obj.snmp_cfg)
	if obj.action is not None:
		output['action'] = obj.action
	if obj.icmp_cfg is not None:
		output['icmp-cfg'] = serialize_Ve__icmp_cfg_json(obj.icmp_cfg)
	if obj.icmpv6_cfg is not None:
		output['icmpv6-cfg'] = serialize_Ve__icmpv6_cfg_json(obj.icmpv6_cfg)
	if obj.ip is not None:
		output['ip'] = serialize_Ve__ip_json(obj.ip)
	if obj.ipv6 is not None:
		output['ipv6'] = serialize_Ve__ipv6_json(obj.ipv6)
	if obj.ddos is not None:
		output['ddos'] = serialize_Ve__ddos_json(obj.ddos)
	return output
...
class Ve__snmp_cfg(AxapiObject):
...
class Ve__icmp_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__icmpv6_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__ip__address(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__ipv6__nat(AxapiObject):
	__metaclass__ = StructMeta 
	inside=PosRangedInteger(0, 1)
	outside=PosRangedInteger(0, 1)
	def __init__(self, inside=None, outside=None):
...
class Ve__ipv6__ndisc__router_adver__mtu(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__ipv6__ndisc__router_adver__vrid(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__ipv6__ndisc__router_adver__ha_group_id(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__ipv6__ndisc__router_adver(AxapiObject):
	__metaclass__ = StructMeta 
...
class Ve__ipv6__ndisc(AxapiObject):
	__metaclass__ = StructMeta 
...
class Interface_ve_address_val_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Interface_ve_address_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Interface_ve_prefix_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class InterfaceVeClient(AbstractResourceClient):
	...
	def getInterfaceVe(self, ve_ifnum):
		"""
		...
		conn.request('GET', self.get_path() + '/' + str(ve_ifnum) .replace("/", "%2f") + query, headers=headers)
	...
	def putInterfaceVe(self, ve_ifnum, ve):
		"""
		...
		conn.request('PUT', self.get_path() + '/' + str(ve_ifnum) .replace("/", "%2f") + query, payload, headers)
		response = conn.getresponse()
		expected_status = 200
	...
	def deleteInterfaceVe(self, ve_ifnum):
		"""
		...
		conn = self.get_connection()
		headers = { 'Content-type' : 'application/json', 'Authorization' : 'A10 %s' %self.sessionid}
		conn.request('DELETE', self.get_path() + '/' + str(ve_ifnum) .replace("/", "%2f") + query, headers=headers)
		response = conn.getresponse()
		expected_status = 200
		errors = {500: 'An unexpected runtime exception', 404: 'Specified ve does not exist'}
		payload = self.get_output(response, expected_status, errors)
...
class AllInterfaceVesClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
