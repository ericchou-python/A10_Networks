...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/lacp',
]
...
def deserialize_Lacp_json(obj):
	...
	if isinstance(obj, dict):
		...
		data = json.loads(obj)
	system_priority = data.get('system-priority')
	result = Lacp(system_priority=system_priority)
	return result

def serialize_Lacp_json(obj):
	output = OrderedDict()
	if obj.system_priority is not None:
		output['system-priority'] = obj.system_priority
	return output
...
class Lacp(AxapiObject):
	...
	system_priority=PosRangedInteger(1, 65535)
	def __init__(self, system_priority=None):
		self.system_priority = system_priority

	def __str__(self):
		return ""

class LacpClient(AbstractResourceClient):
...
class AllLacpsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
