########################################################################################################################
# File name: interface_ve_ipv6.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/interface/ve/ipv6',
]

def deserialize_Interface_ve_ipv6_address_cfg_json(obj):
	if obj is None:
		return None
	...
	result = Interface_ve_ipv6_address_cfg(ipv6_addr=ipv6_addr, anycast=anycast, link_local=link_local)
	return result

def deserialize_list_Interface_ve_ipv6_address_cfg_json(obj):
	if obj is None:
		return None
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_ve_ipv6_address_cfg_json(item))
...
def deserialize_Interface_ve_ipv6_prefix_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, dict):
		data = obj
	...
	result = Interface_ve_ipv6_prefix_cfg(prefix=prefix, not_autonomous=not_autonomous, not_on_link=not_on_link, preferred_lifetime=preferred_lifetime, valid_lifetime=valid_lifetime)
	return result

def deserialize_list_Interface_ve_ipv6_prefix_cfg_json(obj):
	if obj is None:
		return None
	if isinstance(obj, list):
		data = obj
	...
	container = []
	for item in data:
		container.append(deserialize_Interface_ve_ipv6_prefix_cfg_json(item))
	return list(container)
...
def deserialize_Ipv6__ndisc__router_adver_json(obj):
	...
	prefix_cfg = deserialize_list_Interface_ve_ipv6_prefix_cfg_json(data.get('prefix-cfg'))
...
def deserialize_Ipv6_json(obj):
	...
	if isinstance(obj, dict):
		...
		data = json.loads(obj)
	address_cfg = deserialize_list_Interface_ve_ipv6_address_cfg_json(data.get('address-cfg'))
...
def serialize_Interface_ve_ipv6_address_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_ve_ipv6_address_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_ve_ipv6_address_cfg_json(item))
	return output
...
def serialize_Interface_ve_ipv6_prefix_cfg_json(obj):
	output = OrderedDict()
...
def serialize_list_Interface_ve_ipv6_prefix_cfg_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Interface_ve_ipv6_prefix_cfg_json(item))
	return output
...
def serialize_Ipv6__ndisc__router_adver_json(obj):
	...
	if obj.prefix_cfg is not None:
		output['prefix-cfg'] = serialize_list_Interface_ve_ipv6_prefix_cfg_json(obj.prefix_cfg)
...
def serialize_Ipv6_json(obj):
	...
	if obj.address_cfg is not None:
		output['address-cfg'] = serialize_list_Interface_ve_ipv6_address_cfg_json(obj.address_cfg)
...
class Interface_ve_ipv6_address_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class Interface_ve_ipv6_prefix_cfg(AxapiObject):
	__metaclass__ = StructMeta 
...
class InterfaceVeIpv6Client(AbstractResourceClient):
	...
	def putInterfaceVeIpv6(self, ipv6):
		"""
	...
	def deleteInterfaceVeIpv6(self):
		"""
...
class AllInterfaceVeIpv6sClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitInterfaceVeIpv6(self, ipv6):
		"""
	...
	def getAllInterfaceVeIpv6s(self):
		"""
