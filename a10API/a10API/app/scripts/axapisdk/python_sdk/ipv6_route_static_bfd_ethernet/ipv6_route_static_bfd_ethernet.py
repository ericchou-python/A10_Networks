########################################################################################################################
# File name: ipv6_route_static_bfd_ethernet.py
...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/ipv6/route/static/bfd/ethernet',
]
...
def deserialize_Ethernet_json(obj):
	...
	result = Ethernet(eth_num=eth_num, nexthop_ipv6_ll=nexthop_ipv6_ll)
	return result
...
class Ethernet_eth_num_nexthop_ipv6_ll(AxapiObject):
...
class Ipv6RouteStaticBfdEthernetClient(AbstractResourceClient):
	...
	def getIpv6RouteStaticBfdEthernet(self, ethernet_eth_num_nexthop_ipv6_ll):
		"""
		Retrieve the ethernet identified by the specified identifier
		
		Args:
			ethernet_eth_num_nexthop_ipv6_ll Ethernet_eth_num_nexthop_ipv6_ll
		...
		conn.request('GET', self.get_path() + '/' + str(ethernet_eth_num_nexthop_ipv6_ll) .replace("/", "%2f") + query, headers=headers)
	...
	def putIpv6RouteStaticBfdEthernet(self, ethernet_eth_num_nexthop_ipv6_ll, ethernet):
		"""
		...
		conn.request('PUT', self.get_path() + '/' + str(ethernet_eth_num_nexthop_ipv6_ll) .replace("/", "%2f") + query, payload, headers)
		response = conn.getresponse()
		expected_status = 200
	...
	def deleteIpv6RouteStaticBfdEthernet(self, ethernet_eth_num_nexthop_ipv6_ll):
		"""
		...
		conn = self.get_connection()
		headers = { 'Content-type' : 'application/json', 'Authorization' : 'A10 %s' %self.sessionid}
		conn.request('DELETE', self.get_path() + '/' + str(ethernet_eth_num_nexthop_ipv6_ll) .replace("/", "%2f") + query, headers=headers)
		response = conn.getresponse()
		expected_status = 200
		errors = {500: 'An unexpected runtime exception', 404: 'Specified ethernet does not exist'}
		payload = self.get_output(response, expected_status, errors)
...
class AllIpv6RouteStaticBfdEthernetsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitIpv6RouteStaticBfdEthernet(self, ethernet):
		"""
	...
	def getAllIpv6RouteStaticBfdEthernets(self):
		"""
