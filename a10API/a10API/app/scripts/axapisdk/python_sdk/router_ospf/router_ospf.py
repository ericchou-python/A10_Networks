...
BASE_URL = [
	'https://axapi.a10networks.com/axapi/v3/router/ospf',
]
...
def serialize_Router_ospf_area_json(obj):
	output = OrderedDict()
	return output
...
def serialize_list_Router_ospf_area_json(obj):
	output = list()
	for item in obj:
		output.append(serialize_Router_ospf_area_json(item))
	return output
...
def serialize_Ospf_json(obj):
	...
	if obj.arealist is not None:
		output['areaList'] = serialize_list_Router_ospf_area_json(obj.arealist)
...
class RouterOspfClient(AbstractResourceClient):
...
class AllRouterOspfsClient(AbstractResourceClient):
	def __init__(self, endpoint=None, sessionid=None, ipaddress=None, debug=False):
		if not endpoint:
			if ipaddress:
				result = urlparse(BASE_URL[0])
	...
	def submitRouterOspf(self, ospf):
		"""
